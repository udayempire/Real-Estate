...
export default function PropertyPage() {
        ...
        const loadProperty = async () => {
            try {
                ...
                const [response, bookmarkIds] = await Promise.all([
                    api.get<PropertyResponse>(`/staff/properties/exclusive/${exclusivePropertyId}`),
                    fetchBookmarkedPropertyIds(),
                ])
                if (!isMounted) return
                setProperty(response.data.data)
                setIsBookmarked(bookmarkIds.has(response.data.data.sourceProperty.id))
            } catch (err) {
                if (!isMounted) return
    ...
    const handleToggleBookmark = async () => {
        if (!property) return
        const sourcePropertyId = property.sourceProperty.id
        const previous = isBookmarked
        setIsBookmarked(!previous)
        try {
            const nowBookmarked = await toggleBookmark(sourcePropertyId, previous)
    ...
    return (
        <div>
            <h1 className="font-medium text-xl p-2 pl-4">Exclusive Property Details</h1>
